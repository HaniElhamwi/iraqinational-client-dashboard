...
import { ProductSearch, useGetCategories, useGetUsers } from '@/hooks';
...
const Category = () => {
  ...
  const { data, isLoading } = useGetUsers(search?.page, search.limit, search?.search, search.direction);
    ...
    <div>
      ...
      <div className="panel mt-6">
        <div className="mb-5 flex flex-col gap-5 md:flex-row md:items-center">
          <h5 className="text-lg font-semibold dark:text-white-light">{t('users.users')}</h5>
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                ...
                title: t('companyTaxNumber'),
                sortable: true,
                render: ({ companyTaxNumber }) => <div>{companyTaxNumber}</div>,
              },
              {
              ...
              },
              {
                ...
                sortable: true,
                render: ({ verified }) => (
                  <div className={`badge bg-${verified ? 'success' : 'danger'} w-full lg:w-2/4 min-w-min text-center`}>
                    {verified ? 'verified' : 'inActive'}
                  </div>
                ),
              },
              {
                accessor: 'situation',
                title: t('users.situation'),
                sortable: true,
                render: ({ isDisabled }) => (
                  <div className={`badge bg-${!isDisabled ? 'success' : 'danger'} w-full lg:w-2/4 min-w-min text-center`}>
                    {isDisabled ? 'blocked' : 'unblocked'}
                  </div>
                ),
              },
              {
            ...
            totalRecords={parseInt(data?.count)}
